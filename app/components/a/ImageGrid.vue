...
const hovered = ref<number | boolean | null>(null)
</script>

<template>
  <div class="min-h-screen px-4 py-20 md:px-6">
    <div class="columns-1 gap-4 space-y-4 transition-all sm:columns-2 md:columns-3 lg:columns-4">
      <Motion
        ...
        :viewport="{ once: true }"
        class="group relative overflow-hidden rounded-2xl shadow-lg transition-all duration-300 ease-in-out"
      ...
      >
        <Motion
          ...
          :src="src"
          :alt="`${index}`"
          class="w-full rounded-lg object-cover transition-all duration-300 ease-in-out"
          :class="hovered===null ? 'blur-none' : hovered===index ? 'blur-none' : 'blur-xs'"
